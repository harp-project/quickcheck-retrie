...
parseRewriteSpecOptions = concat <$> traverse many
  ...
  , fmap AdhocWithTesting $ option str $ mconcat ----------------------------------------------------------------------------------------------------------------
      [ long "adhoc-with-testing"
      , metavar "EQUATION"
      , help "Apply an adhoc equation of the form: forall vs. lhs = rhs. Checks whether lhs and rhs are semantically the same. Rewrite happens if they are the same."
      ]
